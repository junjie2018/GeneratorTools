    ...
    private void loadTemplate(String projectName) {
        ...
        try {
            for (Map.Entry<String, TemplateConfigsProperties.TemplateConfig> entry
                ...
                TemplateConfigsProperties.TemplateConfig templateConfig = entry.getValue();

                File templateFile = FILENAME_TO_FILE_PATH.get(templateConfig.getTemplate()).toFile();

                FileInputStream fis = new FileInputStream(templateFile);

                byte[] templateBytes = new byte[(int) templateFile.length()];
